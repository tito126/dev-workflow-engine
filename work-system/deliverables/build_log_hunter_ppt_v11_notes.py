...
notes_texts = {
    ...
    4: "这一页我想先把项目背景和目标讲清楚。\n\n我们在实际工作里观察到，系统运行中的很多问题发现仍偏事后响应。也就是说，日志虽然一直在产生，但很多时候并没有真正转化成可以提前发现风险、持续推动优化的抓手。\n\n所以我们做这个项目，不只是为了把日志拉出来看，而是希望建立一套可复用的日志巡检能力，把问题发现、问题分析、优化建议和后续验证串起来，真正服务于健壮性提升、性能优化和日志质量改进。",
    ...
    11: "最后讲一下未来规划。\n\n后续我们会继续围绕三个方向推进：第一，继续增强健壮性、性能提升和日志质量分析能力；第二，把日志反哺建议和整改动作更紧密地连接起来，逐步建立更清晰的验证闭环；第三，在更多环境和更多日志类型中复制这套能力，并继续拓展包括 GC 日志在内的更多分析场景。",
}
