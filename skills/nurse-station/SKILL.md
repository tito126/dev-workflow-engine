...
8. **执行器定位**：实现阶段默认服务于 `codex` / `opencode` / `cc` 等编码工具，不再写“交给当前会话执行”的话术
9. **异常先回退阶段**：若缺少任务号、目标仓未命中、基线同步失败、验证证据不足或执行器结果不完整，先回退到对应阶段补齐，不得硬判完成
