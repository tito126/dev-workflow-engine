...
- 要求结构化输出。
- 保持每个执行器聚焦于单一角色。
- 如果使用 `opencode`，优先以 `ACP opencode` 作为标准路径。
- 当前 Windows 环境下，只有在 `2026-04-08` 记录的本地 ACP 手动补丁仍然存在时，才把 `ACP opencode` 视为正式可用。
- 如果 ACP 不可用、不健康，或升级后补丁被覆盖，则降级到裸 `exec opencode`。
...
当选择 `opencode` 时，优先用 `ACP opencode`，若 ACP 不可用或本地补丁失效 / 被覆盖，则降级到裸 `exec opencode`。
