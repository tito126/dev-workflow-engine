...
### 5. getServiceLogs 支持 endTime 和 type=gc 🟡 待工具组修复
- **endTime**：传了会导致查不到数据（接口 bug，2026-03-14 确认），待工具组修复后在 `_get_download_url` 里取消注释启用
- **type=gc**：后续扩展支持 GC 日志专项分析
- **涉及文件**：`tool_api_fetcher.py`（`_get_download_url` 里有 TODO 注释）
