...
with open(r'C:\Users\pc\.openclaw\workspace\skills\log-inspect\logs_20260311_163218_reanalyzed_final14', encoding='utf-8') as f:
    d = json.load(f)
...
for error_group in d['errors']:
    for sample in error_group['samples']:
        if sample.get('trace_id') == trace_id:
            ...
            print(f"\nError Group Info:")
            print(f"  Group API Entry: {error_group.get('api_entry', 'N/A')}")
            print(f"  Group Count: {error_group['count']}")
            found = True
            break
    if found:
        break
