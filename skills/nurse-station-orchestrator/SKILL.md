...
强制规则：
1. 如果用户已经明确给了仓库路径，可以先使用用户给定路径作为首扫根。
2. 但只要任务可能需要扩扫到其他仓库、前端仓、公共仓、配置仓，就必须先读取上面的 YAML。
3. 如果 YAML 不存在、`status` 不是 `ready`、必填项为空、仍保留占位值、或当前任务无法映射到明确扫描根，则**禁止继续后续阶段**。
4. 此时必须明确要求用户先维护 YAML，而不是依赖记忆、历史经验或临时猜目录继续推进。
5. 只有“用户明确给了唯一路径，且任务确认只在该路径内完成”的 light 级一次性修改，才允许不依赖 YAML 扩扫。

换句话说：
- **允许**从用户明确给的单一路径开始首扫
- **禁止**在 YAML 未配置完成时，擅自扩扫到其他仓库
