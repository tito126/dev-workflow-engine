...
async function login(baseUrl, username, password) {
  const data = await request(`${baseUrl}api/v1/wxpclient/login`, {
    method: "POST",
  ...
  let nameNodeAddrs = null;
  let tenantStatusQueryServers = null;
...
async function getTenants(baseUrl, idUser) {
  const url = withParams(`${baseUrl}api/v2/wxpclient/getUserTenants`, { idUser });
  const data = await request(url, { method: "GET" });
  return data?.data || [];
}
async function getTenantLinks(baseUrl, usercode, idBranch) {
...
var TYPE_LABELS = { 0: "\u8FD0\u7EF4\u5E73\u53F0", 1: "\u4EA4\u4ED8\u5E73\u53F0", 2: "\u95E8\u6237Portal", 3: "\u5176\u4ED6" };
function findServiceLink(links, internalIp, serviceKey, keywords, commonPorts) {
  ...
  for (const keyword of keywords) {
    const matched = links.find(
      (l) => l.relIp === internalIp && (l.simpleName?.toLowerCase().includes(keyword) || l.name?.toLowerCase().includes(keyword))
    );
    if (matched) {
      return { key: serviceKey, name: serviceName, targetPort: matched.relPort, localPort: matched.localPort, source: "matched" };
    }
  ...
  for (const keyword of keywords) {
    const matched = links.find(
      (l) => l.simpleName?.toLowerCase().includes(keyword) || l.name?.toLowerCase().includes(keyword)
    );
    if (matched) {
      return { key: serviceKey, name: serviceName, targetPort: matched.relPort, localPort: matched.localPort, source: "matched" };
    }
...
// src/lockjaw-client.ts
async function request2(baseUrl, apiKey, path, options = {}) {
...
var LockjawError = class extends Error {
  statusCode;
...
async function createChannel(baseUrl, apiKey, params) {
  const { tenantName, hostId, userName, targetIp, targetPort, channelType = "TCP", listenPort, description } = params;
  const data = await request2(baseUrl, apiKey, "/api/v1/lockjaw/channels", {
    method: "POST",
    body: JSON.stringify({
      tenantName,
      hostId,
...
async function getChannels(baseUrl, apiKey) {
  const data = await request2(baseUrl, apiKey, "/api/v1/lockjaw/channels", {
    method: "GET"
  });
  return data.data || [];
}
async function deleteChannel(baseUrl, apiKey, channelId) {
  await request2(baseUrl, apiKey, `/api/v1/lockjaw/channels/${channelId}`, {
    method: "DELETE"
  });
  return true;
}
...
function getConfigDir() {
  return (0, import_node_path.resolve)((0, import_node_os.homedir)(), ".cache", "wxp-tunnel");
}
function getConfigPath() {
  return (0, import_node_path.resolve)(getConfigDir(), "config.json");
}
function getProxyHost(baseUrl) {
  try {
...
function loadConfig() {
  ...
  if (!(0, import_node_fs.existsSync)(configPath)) {
    ...
    (0, import_node_fs.writeFileSync)(configPath, JSON.stringify(defaultConfig, null, 2));
...
async function getTenantList(config, user) {
  const tenantDTOs = await getTenants(
    config.platform.baseUrl,
    user.id_user
  );
  return tenantDTOs.map((item) => ({
    id: item.id_tenant,
    name: item.name,
    code: item.code,
...
function formatLinks(links) {
  return links.map((item) => {
    ...
    const { type, url } = getActualURL(row);
    row.type = type;
    row.typeLabel = TYPE_LABELS[type] || "\u5176\u4ED6";
    row.url = url;
    return row;
  }).sort((a, b) => a.type - b.type);
}
async function getServicePorts(config, tenantCode) {
  ...
  const links = await getTenantLinks(
    config.platform.baseUrl,
    user.code,
    tenant.branchId
  );
  ...
  const promService = findServiceLink(
    formattedLinks,
    internalIp,
    "prometheus",
    ["prometheus", "prom"],
  ...
  const influxService = findServiceLink(
    formattedLinks,
    internalIp,
    "influxdb",
    ["influx"],
  ...
  const services = [opsService, promService, influxService];
  const result = {
    tenantCode,
    internalIp,
    proxyHost,
...
program2.name("wxp-tunnel").description("WXP Tunnel - \u4F7F\u7528 Lockjaw \u4EE3\u7406\u670D\u52A1\u7BA1\u7406 TCP \u96A7\u9053").version("2.0.0");
program2.command("tenants").description("\u5217\u51FA\u6240\u6709\u53EF\u7528\u79DF\u6237").argument("[keyword]", "\u53EF\u9009\u7684\u7B5B\u9009\u5173\u952E\u5B57\uFF0C\u5339\u914D\u79DF\u6237\u540D\u79F0\u6216\u7F16\u7801").option("-k, --keyword <keyword>", "\u53EF\u9009\u7684\u7B5B\u9009\u5173\u952E\u5B57\uFF0C\u5339\u914D\u79DF\u6237\u540D\u79F0\u6216\u7F16\u7801\uFF08\u4E0E\u4F4D\u7F6E\u53C2\u6570\u7B49\u6548\uFF09").action(async (keyword, options) => {
  try {
...
program2.command("links").description("\u5217\u51FA\u6307\u5B9A\u79DF\u6237\u7684\u6240\u6709\u53EF\u7528\u8FDE\u63A5").argument("<tenantCode>", "\u79DF\u6237\u7F16\u7801").action(async (tenantCode) => {
  try {
    const config = loadConfig();
    const user = await ensureLogin(config);
    const tenants = await getTenantList(config, user);
...
program2.command("service-ports").description("\u83B7\u53D6\u79DF\u6237\u7684\u4E09\u4E2A\u76D1\u63A7\u670D\u52A1\u7AEF\u53E3\u6620\u5C04\uFF08\u8FD0\u7EF4\u5E73\u53F0\u3001Prometheus\u3001InfluxDB\uFF09").argument("<tenantCode>", "\u79DF\u6237\u7F16\u7801").action(async (tenantCode) => {
  try {
    const config = loadConfig();
...
program2.command("open-services").description("\u81EA\u52A8\u6253\u5F00\u79DF\u6237\u7684\u4E09\u4E2A\u76D1\u63A7\u670D\u52A1\u96A7\u9053\uFF08\u8FD0\u7EF4\u5E73\u53F0\u3001Prometheus\u3001InfluxDB\uFF09").argument("<tenantCode>", "\u79DF\u6237\u7F16\u7801").action(async (tenantCode) => {
  try {
    const config = loadConfig();
      ...
      try {
        if (serviceInfo.status === "RUNNING") {
          results.push({
            ...
            channelId: serviceInfo.channelId || void 0
          });
          continue;
        }
...
program2.command("close").description("\u5173\u95ED\u6307\u5B9A\u7684\u96A7\u9053").argument("<channelId>", "\u901A\u9053 ID").action(async (channelId) => {
  try {
    const config = loadConfig();
    await deleteChannel(
      config.lockjaw.baseUrl,
...
program2.command("status").description("\u67E5\u770B\u5F53\u524D\u6240\u6709\u6D3B\u8DC3\u96A7\u9053").action(async () => {
  try {
    const config = loadConfig();
    const channels = await getChannels(
      config.lockjaw.baseUrl,
